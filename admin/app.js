...
import { createMediaService } from "../src/platform/media/media-service.js";
import { PLATFORM_MEDIA_BUCKET, VENUE_RUNTIME_BUCKET } from "../src/platform/supabase-config.js";
import {
...
async function createRuntime() {
  ...
  const mediaService = createMediaService({ client: supabase, platformBucket: PLATFORM_MEDIA_BUCKET, venueBucket: VENUE_RUNTIME_BUCKET });
  const repos = {
    auth: authRepository,
    venues: new VenueRepository(supabase, { mediaService }),
    exhibitions: new ExhibitionAdminRepository(supabase),
    site: new SiteRepository(supabase),
    media: new MediaAdminRepository(supabase, { service: mediaService }),

import { el, button, statusBadge, field, textInput, selectInput, formValues, formatDate } from "../core/dom.js";
...
function detailsForm(ctx, detail) {
  ...
  const document = normalizeExhibitionCard(detail.card?.draft_value || {}, exhibition);
  const form = el("form", { class: "formGrid" });
  form.append(
    field("Tytuł", textInput("title", document.title, { required: true })),
    field("Slug", textInput("slug", document.slug, { required: true })),
    field("Podtytuł", textInput("subtitle", document.subtitle)),
    field("Kurator", textInput("curator", document.curator)),
    field("Lokalizacja", textInput("location", document.location)),
    field("Status roboczy", selectInput("status", [
      { value: "draft", label: "Draft" }, { value: "scheduled", label: "Scheduled" }, { value: "published", label: "Published" }, { value: "hidden", label: "Hidden" }
    ], exhibition.status || document.status || "draft")),
    field("Krótki opis", textInput("shortDescription", document.shortDescription, { multiline: true, class: "full" })),
    field("Pełny opis", textInput("longDescription", document.longDescription, { multiline: true, class: "full" })),
    field("Tekst przycisku", textInput("buttonLabel", document.buttonLabel || "Enter gallery")),
    field("Kolejność", textInput("displayOrder", document.displayOrder || 0, { type: "number" })),
    field("Data rozpoczęcia", textInput("startDate", document.startDate ? new Date(document.startDate).toISOString().slice(0,16) : "", { type: "datetime-local" })),
    field("Data zakończenia", textInput("endDate", document.endDate ? new Date(document.endDate).toISOString().slice(0,16) : "", { type: "datetime-local" })),
    el("div", { class: "actions full" }, [button("Zapisz draft dokumentu", { class: "primary", type: "submit" })])
  );
  form.addEventListener("submit", async (event) => {
    event.preventDefault();
    try {
      ...
      await ctx.repos.exhibitions.update(exhibition.id, data, detail.card?.draft_revision, detail.card?.lock_version);
      await ctx.repos.media.finalizePending({ ownerType: "exhibition", ownerId: exhibition.id });
      ctx.shell.toast("Dokument wystawy i media zapisane w draft.", "success");
...
function mediaSelector(ctx, exhibition, media, name, current, options) {
  const select = selectInput(name, mediaOptions(media), current || "");
  const file = el("input", { type: "file", attrs: { accept: options.accept || "image/*,.svg" } });
  const upload = button(options.uploadLabel || "Wgraj nowy plik", { class: "small", onClick: async () => {
    if (!file.files?.[0]) { ctx.shell.toast("Wybierz plik.", "error"); return; }
    upload.disabled = true;
    try {
      const result = await ctx.repos.media.uploadExhibitionFile(file.files[0], exhibition.id, {
        entityType: "branding", entityId: exhibition.id, category: `branding/${options.category}`, usageRole: options.usageRole, mediaType: options.mediaType || "image",
        metadata: { title: `${exhibition.title} — ${options.label}` }
      });
      const mediaId = result.mediaId || result.media_id || result.id;
      media.push({ id: mediaId, media_type: options.mediaType || "image", metadata: { title: `${exhibition.title} — ${options.label}` } });
      select.append(el("option", { value: mediaId, text: `${exhibition.title} — ${options.label} · ${String(mediaId).slice(0,8)}`, selected: true }));
      select.value = mediaId;
      file.value = "";
      ctx.shell.toast(`${options.label} wgrany przez centralny Media Service.`, "success");
    } catch (error) { ctx.shell.toast(error.message, "error"); }
    finally { upload.disabled = false; }
  }});
  return field(options.label, el("div", { class: "grid mediaPicker" }, [select, file, el("div", { class: "actions" }, [upload]), el("p", { class: "help", text: "Nowy upload trafia do folderu tej wystawy. Lista powyżej służy do świadomego wyboru istniejącego medium." })]));
}
...
function cardEditor(ctx, detail, media) {
  ...
  const form = el("form", { class: "formGrid" });
  form.append(
    field("Status label", textInput("statusLabel", card.statusLabel || "")),
    field("Alt text okładki", textInput("coverAltText", card.coverAltText || card.title)),
    field("Focal point X (%)", textInput("coverFocalX", card.coverFocalPoint.x, { type: "number", attrs: { min: 0, max: 100 } })),
    field("Focal point Y (%)", textInput("coverFocalY", card.coverFocalPoint.y, { type: "number", attrs: { min: 0, max: 100 } })),
    mediaSelector(ctx, exhibition, media, "coverMediaId", card.coverMediaId, { label: "Cover desktop", category: "cover", usageRole: "exhibition-cover" }),
    mediaSelector(ctx, exhibition, media, "mobileCoverMediaId", card.mobileCoverMediaId, { label: "Cover mobile", category: "mobile-cover", usageRole: "exhibition-mobile-cover" }),
    mediaSelector(ctx, exhibition, media, "logoMediaId", card.logoMediaId, { label: "Logo", category: "logo", usageRole: "exhibition-logo", accept: "image/*,.svg", mediaType: "logo" }),
    field("Kolor akcentu", textInput("themeAccent", card.theme.accent || "", { type: "color" })),
    field("Kolor tła", textInput("themeBackground", card.theme.background || "", { type: "color" })),
    field("Kolor tekstu", textInput("themeText", card.theme.text || "", { type: "color" })),
    field("Styl karty", selectInput("themeCardStyle", ["default", "glass", "minimal"], card.theme.cardStyle || "default")),
    el("div", { class: "actions full" }, [button("Zapisz branding draft", { class: "primary", type: "submit" })])
  );
  form.addEventListener("submit", async (event) => {
    event.preventDefault();
    try {
      const data = { ...card, ...formValues(form) };
      data.coverFocalPoint = { x: Number(data.coverFocalX || 50), y: Number(data.coverFocalY || 50) };
      data.theme = { accent: data.themeAccent || "", background: data.themeBackground || "", text: data.themeText || "", cardStyle: data.themeCardStyle || "default" };
      ...
      const mediaRoles = {
        coverMediaId: "exhibition-cover",
        mobileCoverMediaId: "exhibition-mobile-cover",
        logoMediaId: "exhibition-logo"
      };
      for (const [key, usageRole] of Object.entries(mediaRoles)) {
        const mediaId = validation.value[key];
        if (!mediaId) continue;
        await ctx.repos.media.attachExisting(mediaId, {
          ownerType: "exhibition",
          ownerId: exhibition.id,
          entityType: "exhibition-document",
      ...
      await ctx.repos.media.finalizePending({ ownerType: "exhibition", ownerId: exhibition.id });
      ctx.shell.toast("Branding draft i media zapisane.", "success");
      await renderExhibitionDetail(ctx, exhibition.id);
    } catch (error) { ctx.shell.toast(error.message, "error"); }
  });
  return form;

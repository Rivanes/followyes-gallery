/*
  Berryboy Art Gallery — CMS Runtime Integration
...
import { SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, PLATFORM_MEDIA_BUCKET, VENUE_RUNTIME_BUCKET } from "../platform/supabase-config.js";
import { createMediaService } from "../platform/media/media-service.js";
...
import {
  ...
  ExhibitionMediaUsageRepository,
  AuthorRepository,
  buildExhibitionUrl,
  createControlledRestartController
} from "../runtime/exhibition-runtime.js";

const RUNTIME_RELEASE = "cms-runtime-integration";
const ENGINE_CACHE_KEY = "gallery_engine_cms_runtime";
...
let platformMediaService = null;
let authorRepository = null;
...
async function hydrateVenueManifestAssetUrls(manifestValue) {
  const manifest = typeof structuredClone === "function" ? structuredClone(manifestValue) : JSON.parse(JSON.stringify(manifestValue));
  const assets = Array.isArray(manifest.assets) ? manifest.assets : [];
  await Promise.all(assets.map(async (asset) => {
    if (!asset || asset.publicUrl || asset.url || !asset.storagePath) return;
    const bucket = asset.storageBucket || VENUE_RUNTIME_BUCKET;
    const response = await supabase.storage.from(bucket).createSignedUrl(asset.storagePath, 3600);
    if (response.error) throw response.error;
    asset.publicUrl = response.data && response.data.signedUrl || "";
  }));
  return manifest;
}
...
async function prepareGalleryRuntimeContext() {
  ...
  if (!selected.manifest && !selected.manifestUrl) throw new Error("The selected Venue Version has no published manifest. Complete the D3R database migration and Venue publication first.");
  const loadedManifest = selected.manifest || await loadVenueManifest(selected.manifestUrl);
  const manifest = await hydrateVenueManifestAssetUrls(loadedManifest);
  ...
  mediaRepository = new ExhibitionMediaUsageRepository({
    supabase,
    bucket: PLATFORM_MEDIA_BUCKET,
    exhibitionId: selected.id
  });
  platformMediaService = createMediaService({
    client: supabase,
    platformBucket: PLATFORM_MEDIA_BUCKET,
    venueBucket: VENUE_RUNTIME_BUCKET
  });
  authorRepository = new AuthorRepository({ supabase, exhibitionId: selected.id });
...
function installResizeRuntime(engine) {
  // Stage C6C1: mobile DPR and resize are owned by gallery-engine through the
...
async function startGalleryRuntime() {
  ...
  galleryStartPromise = (async function () {
    ...
    // Hide the page loader first, then show and verify the preserved engine-owned intro popup.
    bootGuard.ready();
    window.requestAnimationFrame(function () {
      ...
      window.requestAnimationFrame(function () {
        ...
        window.dispatchEvent(new CustomEvent("gallery-instruction-popup-confirmed", {
          detail: { release: RUNTIME_RELEASE, confirmedAt: Date.now() }
        }));
      });
    });

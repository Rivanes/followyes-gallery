...
const controls = {
  ...
  '[data-control="mobile-look"]': new FakeElement(),
...
for (const button of [lookMiddle, lookRight]) {
  ...
  button.children['[data-gallery-look-label]'] = new FakeElement();
  button.children['[data-gallery-look-hint]'] = new FakeElement();
}
...
assert(elements.galleryBootTitle.textContent === 'Zwiedzaj galerię', 'Prestart title incorrect');
assert(lookMiddle.children['[data-gallery-look-hint]'].textContent.includes('kółko'), 'Restored mouse instruction hint missing');
...
console.log('Stage 12C66B1 startup state-machine test passed.');

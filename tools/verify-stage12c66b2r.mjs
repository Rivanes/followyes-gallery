...
const txt = fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66B2R_REPAIR_LOGIN_DISABLED.txt', import.meta.url), 'utf8');
...
assert(index.includes('Stage 12C66B2R'), 'Index Stage 12C66B2R label missing');
assert(index.includes('stage: "12C66B2R"'), 'Boot Guard Stage 12C66B2R identity missing');
assert(source.includes('Stage 12C66B2R: Save Integrity Repair / Correct Startup Rebuild'), 'Stage 12C66B2R source history missing');
assert(source.includes('stage: "12C65E"'), 'Protected Stage 12C65E streaming runtime identity missing');
assert(source.includes('stage: "12C66B2R"'), 'Stage 12C66B2R save-integrity runtime identity missing');
assert(bootstrap.includes('Stage 12C66B2R'), 'Viewer bootstrap Stage 12C66B2R label missing');
assert(bootstrap.includes('stage12c66b2r_repair_20260723'), 'Stage 12C66B2R engine cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66b2r_repair_20260723'), 'Page bootstrap Stage 12C66B2R cache key missing');
assert(bootstrap.includes('const STAGE = "12C66B2R"'), 'Viewer runtime Stage 12C66B2R identity missing');
assert(editorBootstrap.includes('Stage 12C66B2R'), 'Editor bootstrap Stage 12C66B2R label missing');
...
// Stage 12C66B2R save-integrity repair contract.
assert(source.includes('var gallerySaveIntegrityRuntime = {'), 'Save integrity runtime missing');
assert(source.includes('schema: "gallery-save-integrity.v2"'), 'Save integrity v2 schema missing');
...
assert(minified.includes('12C66B2R'), 'Production Stage 12C66B2R identity missing');
...
console.log('Stage 12C66B2R verifier passed.');

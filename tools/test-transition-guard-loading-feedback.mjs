...
expect('current package keeps transition guard regression coverage', pkg.version.includes('c6c8c14') && viewer.includes('const STAGE = "12C66C6C8C14"') && admin.includes('const STAGE = "12C66C6C8C14"'));
...
expect('Admin to Public clean same-runtime return bypasses the full-page guard with guarded fallback', viewer.includes('const instantFastPath = !sceneDirty && canUseInstantWorkspaceModeSwitch()') && viewer.includes('if (!instantFastPath)') && viewer.includes('title: "Returning to Public Page…"'));

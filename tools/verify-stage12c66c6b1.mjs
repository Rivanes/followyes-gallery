...
const txt = fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66C6B1_EXISTING_AUTHOR_AVIF_RECONCILIATION_LOGIN_DISABLED.txt', import.meta.url), 'utf8');
...
assert(index.includes('stage: "12C66C6B1"'), 'Boot Guard C5 identity missing');
assert(bootstrap.includes('const STAGE = "12C66C6B1"'), 'Viewer runtime C5 identity missing');
assert(bootstrap.includes('stage12c66c6b1_existing_author_avif_reconciliation_20260724'), 'C6B1 cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66c6b1_existing_author_avif_reconciliation_20260724'), 'Index C6B1 cache key missing');
assert(editorBootstrap.includes('Stage 12C66C6B1'), 'Editor bootstrap C6B label missing');
assert(source.includes('Stage 12C66C6A: Mobile Quality Foundation / Artwork Always Visible'), 'C6A source history missing');
assert(source.includes('Stage 12C66C6A1: Inspect Transition Isolation / Compact Mobile Inspect UI'), 'C6A1 source history missing');
assert(source.includes('Stage 12C66C6B: AVIF Pipeline / Migration / WebP Removal'), 'C6B source history missing');
assert(source.includes('stage: "12C66C6B1"'), 'C6B runtime identity missing');
...
assert(minified.includes('12C66C6B1') && minified.includes('gallery-ground-collision.v1') && minified.includes('gallery-sculpture-core.v2'), 'C5 runtime missing from production build');

// Stage 12C66C6A1 — artwork visibility and lifecycle foundation.
...
const fullBusy = extractFunction(source, 'isGalleryViewerBusyForFullArtworkUpgrade');
assert(fullBusy.includes('isGalleryInspectCameraTransitionActive') && fullBusy.includes('viewerMoveKeys') && fullBusy.includes('mobileJoystickActive'), 'Full artwork upgrades are not isolated from transition/movement');
...
// Stage 12C66C6A1 — Inspect transition isolation and compact mobile UI.
const openInspect = extractFunction(source, 'openGalleryInspectTarget');
assert(openInspect.indexOf('completeGalleryInspectCameraTransition') < openInspect.indexOf('prioritizeArtworkFullTexture'), 'Full texture priority occurs before Inspect settles');
assert(!openInspect.slice(0, openInspect.indexOf('var startFocus')).includes('prioritizeArtworkFullTexture'), 'Inspect still starts Full load before focus');
assert(extractFunction(source, 'drainGalleryFastStartFullArtworkQueue').includes('_galleryTextureOnlyUpgrade = true'), 'Preview -> Full is not texture-only');
const artworkApply = extractFunction(source, 'applyArtworkImageState');
assert(artworkApply.includes('recordArtworkTextureDimensionsWithoutGeometry') && artworkApply.includes('if (!textureOnlyUpgrade)'), 'Full texture can still mutate artwork geometry');
assert(source.includes('function armGalleryInspectTransitionWatchdog('), 'Inspect watchdog missing');
assert(source.includes('function recoverGalleryInspectTransition('), 'Inspect recovery path missing');
assert(extractFunction(source, 'beginGalleryInspectCameraTransition').includes('armGalleryInspectTransitionWatchdog'), 'Transition does not arm watchdog');
assert(extractFunction(source, 'completeGalleryInspectCameraTransition').includes('clearGalleryInspectTransitionWatchdog'), 'Transition completion does not clear watchdog');
assert(source.includes('function shouldShowMobileViewerControls('), 'Mobile controls visibility owner missing');
assert(extractFunction(source, 'syncMobileViewerUiVisibility').includes('isGalleryInspectSuppressingMobileViewerControls'), 'Viewport refresh can resurrect joystick during Inspect');
const safeFrame = extractFunction(source, 'updateGalleryMobileInspectSafeFrame');
assert(safeFrame.includes('compact-bottom') && safeFrame.includes('joystickVisible = false'), 'Compact safe-frame does not exclude joystick');
assert(!safeFrame.includes('mobileJoystickBase'), 'Inspect safe-frame still measures joystick');
assert(source.includes('STAGE 12C66C6A1 — COMPACT MOBILE INSPECT CAPSULE'), 'Compact mobile Inspect CSS missing');
assert(source.includes('clip: rect(0, 0, 0, 0) !important'), 'Mobile Previous/Next labels remain visually active');
assert(source.includes('--gallery-inspect-navigation-size: 44px'), 'Compact circular mobile navigation missing');

// Stage 12C66C6B — AVIF pipeline, migration safety and WebP removal.
assert(source.includes('galleryArtworkImageVariantFormat = "image/avif"'), 'AVIF output MIME missing');
assert(source.includes('galleryArtworkImageVariantExtension = "avif"'), 'AVIF output extension missing');
assert(source.includes('@jsquash/avif@2.1.1'), 'Pinned AVIF encoder missing');
assert(source.includes('src/workers/gallery-avif-encoder-worker.js'), 'Lazy AVIF worker missing');
assert(source.includes('upsert: false'), 'Immutable AVIF upload contract missing');
assert(extractFunction(source, 'createArtworkVariantStoragePath').includes('variantSetId'), 'Versioned variant paths missing');
assert(extractFunction(source, 'createArtworkImageVariantBlob').includes('encodeGalleryImageDataToAvif'), 'AVIF encoder is not used');
assert(!extractFunction(source, 'createArtworkImageVariantBlob').includes('canvas.toBlob'), 'Canvas WebP encoder remains active');
assert(extractFunction(source, 'isGalleryAvifVariantComplete').includes('image/avif'), 'AVIF completeness validation missing');
assert(extractFunction(source, 'getArtworkTextureNoMipmap').includes('return false'), 'Artwork mipmaps remain disabled');
assert(extractFunction(source, 'getArtworkTextureAnisotropyLevel').includes('? 8 : 16'), 'Artwork anisotropy quality floor missing');
assert(extractFunction(source, 'applyArtworkImageState').includes('AVIF original fallback'), 'AVIF decode fallback to original missing');
assert(extractFunction(source, 'queueReplacedGalleryArtworkStateForCleanup').includes('Original source files'), 'Original source preservation contract missing');
assert(extractFunction(source, 'scanGalleryLegacyGeneratedWebpStorage').includes('getGalleryLegacyGeneratedWebpPrefixes'), 'Generated WebP Storage audit missing');
assert(extractFunction(source, 'removeGalleryLegacyGeneratedWebpFiles').includes('.remove(batch)'), 'Generated WebP Storage removal missing');
const finalizeAvif = extractFunction(source, 'finalizeGalleryAvifMigrationAndRemoveWebp');
assert(count(finalizeAvif, 'saveGalleryStateToSupabase()') === 2, 'AVIF finalization does not rotate backup with two saves');
assert(finalizeAvif.includes('removeGalleryLegacyGeneratedWebpFiles'), 'AVIF finalization does not remove generated WebP');
assert(source.includes('TEST SELECTED ARTWORK AVIF') && source.includes('AUDIT GENERATED WEBP'), 'AVIF test/audit controls missing');
assert(source.includes('BUILD MISSING ARTWORK AVIF') && source.includes('FORCE REBUILD ARTWORK AVIF'), 'Artwork AVIF controls missing');
assert(source.includes('VALIDATE AVIF MIGRATION') && source.includes('FINALIZE + REMOVE WEBP'), 'Migration controls missing');
assert(source.includes('getAvifMigrationDebug: getGalleryAvifMigrationDebug'), 'AVIF diagnostics API missing');
assert(source.includes('originalWebpSourcesPreserved'), 'Original WebP preservation is not reported');

assert(source.includes('function reconcileExistingAuthorAvifVariants('), 'Existing author AVIF reconciliation missing');
assert(source.includes('function scanGalleryExistingAuthorAvifStorage('), 'Existing author AVIF Storage scan missing');
assert(source.includes('RECONCILE / BUILD AUTHOR AVIF'), 'Author reconciliation UI missing');
const reconcileAuthorAvif = extractFunction(source, 'reconcileExistingAuthorAvifVariants');
assert(reconcileAuthorAvif.includes('verifyGalleryExistingAuthorAvifState'), 'Recovered author AVIF is not verified');
assert(reconcileAuthorAvif.includes('syncAllArtworksForAuthor'), 'Recovered author AVIF is not synchronized');
assert(!reconcileAuthorAvif.includes('createAndUploadAuthorPhotoVariants'), 'Reconciliation re-encodes existing author AVIF');
assert(extractFunction(source, 'finalizeGalleryAvifMigrationAndRemoveWebp').includes('reconcileExistingAuthorAvif: true'), 'Finalization does not reconcile existing author AVIF');

assert(txt.includes('var galleryEditorLoginEnabled = false;'), 'Login-disabled C6B1 build missing');
assert(!txt.includes('var galleryEditorLoginEnabled = true;'), 'Login remains enabled in C6B1 TXT');

console.log('Stage 12C66C6B1 verifier passed.');

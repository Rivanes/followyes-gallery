...
const txt = fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66C5A_UNIFIED_GROUND_COLLISION_LOGIN_DISABLED.txt', import.meta.url), 'utf8');
...
assert(index.includes('stage: "12C66C5A"'), 'Boot Guard C5 identity missing');
assert(bootstrap.includes('const STAGE = "12C66C5A"'), 'Viewer runtime C5 identity missing');
assert(bootstrap.includes('stage12c66c5a_unified_ground_collision_camera_height_restore_20260724'), 'C5 cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66c5a_unified_ground_collision_camera_height_restore_20260724'), 'Index C5 cache key missing');
assert(editorBootstrap.includes('Stage 12C66C5A'), 'Editor bootstrap C5 label missing');
assert(source.includes('Stage 12C66C5A: Camera Height Restore'), 'C5A source history missing');
assert(source.includes('stage: "12C66C5A"'), 'C5 engine/save identity missing');
...
assert(minified.includes('12C66C5A') && minified.includes('gallery-ground-collision.v1') && minified.includes('gallery-sculpture-core.v2'), 'C5 runtime missing from production build');

console.log('Stage 12C66C5A verifier passed.');

...
assert(source.includes('stage: "12C66C6B1"'), 'C6B1 stage identity missing');
assert(source.includes('RECONCILE / BUILD AUTHOR AVIF'), 'Reconciliation UI entry missing');
...
const validation = extractFunction(source, 'validateGalleryAvifMigration');
assert(validation.includes('reconcileExistingAuthorAvifVariants') && validation.includes('authorReconciliation'), 'Validation cannot reconcile existing sets');
const finalize = extractFunction(source, 'finalizeGalleryAvifMigrationAndRemoveWebp');
assert(finalize.includes('reconcileExistingAuthorAvif: true'), 'Finalization does not reconcile before readiness check');
const repairUi = extractFunction(source, 'runAuthorPhotoVariantsRebuildFromUi');
assert(repairUi.indexOf('reconcileExistingAuthorAvifVariants') < repairUi.indexOf('rebuildAllAuthorPhotoVariants'), 'UI rebuilds before reconciliation');
...
assert(source.includes('reconcileExistingAuthorAvif: function'), 'Public reconciliation API missing');
...
console.log('Stage 12C66C6B1 existing author AVIF reconciliation tests passed.');

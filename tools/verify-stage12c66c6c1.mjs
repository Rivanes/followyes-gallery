...
const txt=fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66C6C1_CANONICAL_VISUAL_STATE_MOBILE_LIGHTING_REFLECTION_PARITY_LOGIN_DISABLED.txt',import.meta.url),'utf8');
...
assert(index.includes('stage: "12C66C6C1"'),'Index stage identity missing');
assert(bootstrap.includes('const STAGE = "12C66C6C1"'),'Bootstrap stage identity missing');
assert(bootstrap.includes('stage12c66c6c1_canonical_visual_mobile_lighting_parity_20260725'),'C6C1 cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66c6c1_canonical_visual_mobile_lighting_parity_20260725'),'Index cache key missing');
assert(editorBootstrap.includes('Stage 12C66C6C1'),'Editor bootstrap label missing');
assert(source.includes('Stage 12C66C6C1: Canonical Visual State / Mobile Lighting & Reflection Parity'),'Source history missing');
...
assert(minified.includes('12C66C6C1')&&minified.includes('gallery-atomic-media-lifecycle.v1')&&minified.includes('gallery-mobile-quality-domains.v2')&&minified.includes('gallery-canonical-visual-state.v1'),'C6C1 runtime missing from production build');
...
console.log('Stage 12C66C6C1 verifier passed.');

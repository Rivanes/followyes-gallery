...
const txt = fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66C2_MICRO_STABILIZATION_LOGIN_DISABLED.txt', import.meta.url), 'utf8');
...
assert(index.includes('stage: "12C66C2"'), 'Boot Guard Stage 12C66C2 identity missing');
assert(source.includes('Stage 12C66C2: Micro Stabilization'), 'Stage 12C66C2 source history missing');
assert(source.includes('stage: "12C65E"'), 'Protected Stage 12C65E streaming runtime identity missing');
assert(source.includes('stage: "12C66C2"'), 'Stage 12C66C2 save-integrity runtime identity missing');
assert(bootstrap.includes('Stage 12C66C2'), 'Viewer bootstrap Stage 12C66C2 label missing');
assert(bootstrap.includes('stage12c66c2_micro_stabilization_20260723'), 'Stage 12C66C2 engine cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66c2_micro_stabilization_20260723'), 'Page bootstrap Stage 12C66C2 cache key missing');
assert(bootstrap.includes('const STAGE = "12C66C2"'), 'Viewer runtime Stage 12C66C2 identity missing');
assert(editorBootstrap.includes('Stage 12C66C2'), 'Editor bootstrap Stage 12C66C2 label missing');
...
// Zone-based Props stay active, but Local Lights no longer cull/fade by camera or zone.
...
assert(!source.includes('function shouldLocalLightBeRuntimeEnabled'), 'Automatic Local Light runtime culling remains');
assert(!source.includes('localLightCameraCullingEnabled'), 'Camera-view Local Light culling settings remain');
assert(!source.includes('outsideActiveGalleryZone'), 'Zone-based Local Light shutdown remains');
assert(!source.includes('updateLocalLightsCameraCulling'), 'Per-frame Local Light camera culling remains');
assert(!source.includes('runtimeTargetIntensity'), 'Legacy Local Light fade target state remains');
...
// Stage 12C66C2 save-integrity repair contract.
...
// Stage 12C66C2 input, editor workflow and collision contract.
...
assert(floorPick.includes('scene.pick(x, y, null, false, camera)'), 'Floor cursor still raycasts through visible occluders');
const floorIndicatorConfig = extractFunction(source, 'configureGalleryFloorIndicatorMesh');
assert(floorIndicatorConfig.includes('mesh.renderOutline = true'), 'Floor cursor contrast outline missing');
assert(source.includes('galleryFloorCursorRingMaterial.alpha = 0.94'), 'Floor cursor visibility boost missing');
assert(source.includes('galleryFloorCursorRingMaterial.zOffset = -4'), 'Floor cursor depth bias missing');
assert(source.includes('ring.position.y += 0.060'), 'Floor cursor segment-clearance offset missing');
assert(source.includes('ring.scaling.set(1, 0.10, 1)'), 'Floor cursor flattening missing');
assert(source.includes('GalleryFloorCursorPulseRing'), 'Floor click ripple mesh missing');
assert(source.includes('updateGalleryFloorCursorClickPulse()'), 'Floor click ripple frame update missing');
assert(source.includes('function isGalleryInspectTransitionInteractionLocked()'), 'Inspect transition lock missing');
assert(extractFunction(source, 'updateViewerWASDMovement').includes('clearGalleryInspectTransitionInput();'), 'Viewer input can still cancel Inspect travel');
assert(extractFunction(source, 'updateEditModeMovementFrame').includes('clearGalleryInspectTransitionInput();'), 'Edit input can still cancel Inspect travel');
assert(source.includes('if (isGalleryInspectTransitionInteractionLocked()) {\n            if (evt && evt.preventDefault) evt.preventDefault();'), 'Canvas clicks can still interrupt Inspect travel');
...
const c1AttachControl = extractFunction(source, 'attachGalleryCameraControl');
assert(c1AttachControl.includes('mobileViewerEnabled === true && editMode !== true'), 'Mobile Viewer can still reattach Babylon touch input');
assert(c1AttachControl.includes('camera.detachControl(canvas)'), 'Mobile Viewer attach guard does not detach controls');
assert(c1AttachControl.includes('clearGalleryBuiltInCameraMotionResidue()'), 'Residual built-in camera direction is not cleared');
const c1MobileRefresh = extractFunction(source, 'refreshMobileViewerMode');
assert(c1MobileRefresh.includes('shouldEnable && !editMode'), 'Mobile refresh does not heal accidental camera reattach');
assert(c1MobileRefresh.includes('camera.detachControl(canvas)'), 'Mobile refresh leaves Babylon touch input attached');
const c1MobileBegin = extractFunction(source, 'beginMobileCanvasLook');
assert(c1MobileBegin.includes('if (mobileLookActive)'), 'A second canvas pointer can replace the active gesture owner');
assert(c1MobileBegin.includes('mobileJoystickActive ? "look" : "pending"'), 'Joystick and temporary hold movement can still own movement simultaneously');
const c1Grounded = extractFunction(source, 'enforceMobileViewerGroundedWalkHeight');
assert(c1Grounded.includes('camera.position.y = Number(walkY)'), 'Mobile Viewer walk height is not hard-clamped');
assert(c1Grounded.includes('clearGalleryBuiltInCameraMotionResidue()'), 'Mobile Viewer vertical drift residue is not cleared');
assert(source.includes('mobileNavigationWindowBlurSafety'), 'Mobile blur input reset missing');
assert(source.includes('mobileNavigationPageHideSafety'), 'Mobile pagehide input reset missing');
assert(source.includes('mobileNavigationVisibilitySafety'), 'Mobile hidden-tab input reset missing');
...
assert(minified.includes('12C66C2'), 'Production Stage 12C66C2 identity missing');
...
console.log('Stage 12C66C2 verifier passed.');

...
const txt = fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66B1_SINGLE_STARTUP_CLEAN_VIEWER_LOGIN_DISABLED.txt', import.meta.url), 'utf8');
...
assert(index.includes('Stage 12C66B1'), 'Index Stage 12C66B1 label missing');
assert(index.includes('stage: "12C66B1"'), 'Boot Guard Stage 12C66B1 identity missing');
assert(source.includes('Stage 12C66B1: Restored Instructional Intro / Single Startup Gate'), 'Stage 12C66B1 source history missing');
...
assert(bootstrap.includes('Stage 12C66B1 Restored Instructional Intro / Single Startup Gate'), 'Viewer bootstrap Stage 12C66B1 label missing');
assert(bootstrap.includes('stage12c66b1_intro_restore_20260723'), 'Stage 12C66B1 engine cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66b1_intro_restore_20260723'), 'Page bootstrap Stage 12C66B1 cache key missing');
assert(bootstrap.includes('const STAGE = "12C66B1"'), 'Viewer runtime Stage 12C66B1 identity missing');
assert(editorBootstrap.includes('Stage 12C66B1 Restored Instructional Intro / Single Startup Gate'), 'Editor bootstrap Stage 12C66B1 label missing');
...
// Stage 12C66B1 explicit startup gate and one public loading surface.
...
assert(index.includes('class="berryboyKey">W</span>'), 'Restored animated WASD instructions missing');
assert(index.includes('class="berryboyMouseIcon"'), 'Restored animated mouse instruction missing');
assert(index.includes('class="berryboyJoystickIcon"'), 'Restored animated joystick instruction missing');
assert(index.includes('class="berryboyTapIcon"'), 'Restored animated inspect instruction missing');
assert(index.includes('@keyframes berryboyKeyPulse'), 'Restored instruction animations missing');
assert(index.includes('#galleryBootGuard[data-state="prestart"] #galleryBootControls'), 'Instruction panel is not visible before startup');
...
assert(minified.includes('12C66B1'), 'Production mirror Stage 12C66B1 identity missing');
...
console.log('Stage 12C66B1 verifier passed.');

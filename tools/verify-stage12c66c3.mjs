...
const txt = fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66C3_SYSTEMIC_STABILIZATION_LOGIN_DISABLED.txt', import.meta.url), 'utf8');
...
assert(index.includes('stage: "12C66C3"'), 'Boot Guard Stage 12C66C3 identity missing');
assert(source.includes('Stage 12C66C3: Systemic Stabilization'), 'Stage 12C66C3 source history missing');
assert(source.includes('stage: "12C65E"'), 'Protected Stage 12C65E streaming runtime identity missing');
assert(source.includes('stage: "12C66C3"'), 'Stage 12C66C3 save-integrity runtime identity missing');
assert(bootstrap.includes('Stage 12C66C3'), 'Viewer bootstrap Stage 12C66C3 label missing');
assert(bootstrap.includes('stage12c66c3_systemic_stabilization_20260723'), 'Stage 12C66C3 engine cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66c3_systemic_stabilization_20260723'), 'Page bootstrap Stage 12C66C3 cache key missing');
assert(bootstrap.includes('const STAGE = "12C66C3"'), 'Viewer runtime Stage 12C66C3 identity missing');
assert(editorBootstrap.includes('Stage 12C66C3'), 'Editor bootstrap Stage 12C66C3 label missing');
...
// Zone-based Props remain; Local Lights intentionally stay persistent.
...
assert(source.includes('function applyLocalLightUserState(item)'), 'Persistent Local Light state missing');
assert(!source.includes('shouldLocalLightBeRuntimeEnabled'), 'Obsolete Local Light zone gate returned');
...
// Stage 12C66C3 save-integrity repair contract.
...
// Stage 12C66C3 systemic input, marker, transition and collision contract.
...
for (const direction of ['forward', 'backward', 'turnLeft', 'turnRight']) {
  assert(source.includes(`galleryDesktopDpadState.${direction}`), `D-pad ${direction} state missing`);
}
...
assert(source.includes('body.is-editor-authenticated:not(.gallery-edit-mode-active) #galleryDesktopDpad'), 'Authenticated D-pad offset rule missing');
assert(source.includes('right: calc(var(--gallery-editor-screen-gap, 30px) + 196px)'), 'D-pad still collides with floating Edit Mode button');

// Procedural floor marker: one soft SDF plane, first-visible-floor pick and no torus/outline path.
const floorPick = extractFunction(source, 'pickGalleryFloorFromPointer');
assert(floorPick.includes('floorMeshes.indexOf(visiblePick.pickedMesh) === -1'), 'Floor cursor can target non-floor meshes');
assert(floorPick.includes('scene.pick(x, y, null, false, camera)'), 'Floor cursor raycasts through visible occluders');
const floorIndicatorConfig = extractFunction(source, 'configureGalleryFloorIndicatorMesh');
assert(floorIndicatorConfig.includes('mesh.renderOutline = false'), 'Hard geometric outline returned');
assert(floorIndicatorConfig.includes('mesh.renderingGroupId = 3'), 'Floor cursor overlay rendering group missing');
assert(source.includes('GalleryFloorCursorSdfPlane'), 'Procedural floor cursor plane missing');
assert(source.includes('new BABYLON.ShaderMaterial'), 'Procedural floor cursor shader missing');
assert(source.includes('float softRing(float radius, float halfWidth, float softness)'), 'SDF ring function missing');
assert(source.includes('smoothstep(halfWidth - softness, halfWidth + softness, radial)'), 'Soft antialiased SDF edge missing');
assert(source.includes('depthFunction = BABYLON.Constants.ALWAYS') || source.includes('depthFunction = BABYLON.Engine.ALWAYS'), 'Segment-independent depth overlay missing');
assert(!source.includes('GalleryFloorCursorPulseRing'), 'Old separate pulse torus remains');
const markerFactory = extractFunction(source, 'getOrCreateGalleryFloorCursorRing');
assert(!markerFactory.includes('CreateTorus'), 'Floor cursor still uses a torus');
assert(markerFactory.includes('CreatePlane'), 'Floor cursor plane missing');
assert(source.includes('pulseProgress'), 'Shared shader pulse missing');

// Touch devices never regain native Babylon touch input in public Viewer.
assert(source.includes('function disableGalleryBuiltInTouchCameraMovement()'), 'Native touch-input removal helper missing');
const touchDisable = extractFunction(source, 'disableGalleryBuiltInTouchCameraMovement');
assert(touchDisable.includes('camera.inputs.removeByType("FreeCameraTouchInput")'), 'FreeCameraTouchInput is not removed');
const attachControl = extractFunction(source, 'attachGalleryCameraControl');
assert(attachControl.includes('disableGalleryBuiltInTouchCameraMovement()'), 'Camera reattach can restore native touch input');
assert(attachControl.includes('isGalleryTouchCapableDevice()'), 'Touch-capable desktop-layout viewport is not protected');
const mobileRefresh = extractFunction(source, 'refreshMobileViewerMode');
assert(mobileRefresh.includes('isGalleryTouchCapableDevice()'), 'Viewport refresh can restore native touch input');
assert(mobileRefresh.includes('disableGalleryBuiltInTouchCameraMovement()'), 'Viewport refresh does not remove native touch input');
...
assert(grounded.includes('camera.position.y = Number(walkY)'), 'Mobile Viewer walk height is not hard-clamped');
assert(grounded.includes('clearGalleryBuiltInCameraMotionResidue()'), 'Mobile vertical residue is not cleared');

// Inspect transition owns the camera centrally, including the capture-phase desktop look listener.
assert(source.includes('function isGalleryInspectTransitionInteractionLocked()'), 'Transition interaction lock missing');
const desktopLookAllowed = extractFunction(source, 'isDesktopViewerMiddleLookAllowed');
assert(desktopLookAllowed.includes('isGalleryInspectTransitionInteractionLocked()'), 'Desktop look can start during transition');
const beginDesktopLook = extractFunction(source, 'beginDesktopViewerMiddleLook');
assert(beginDesktopLook.indexOf('isGalleryInspectTransitionInteractionLocked()') < beginDesktopLook.indexOf('closeGalleryInspect("desktop-manual-look")'), 'Capture-phase look closes Inspect before transition guard');
const closeInspect = extractFunction(source, 'closeGalleryInspect');
assert(closeInspect.includes('isGalleryInspectTransitionInteractionLocked() && !isGalleryInspectSystemCloseReason(reason)'), 'Central close guard missing');
const viewerMovement = extractFunction(source, 'updateViewerWASDMovement');
assert(viewerMovement.includes('clearGalleryInspectTransitionInput();\n            return;'), 'Viewer movement can cancel transition');
const editMovement = extractFunction(source, 'updateEditModeMovementFrame');
assert(editMovement.includes('clearGalleryInspectTransitionInput();\n            return;'), 'Edit movement can cancel transition');

// Sculpture proxy is connected to the proven custom resolver, not only native checkCollisions.
...
assert(!extractFunction(source, 'suspendModel3dForStreaming').includes('disableSculptureCollisionProxy(slot)'), 'Streaming disables sculpture collision');
assert(source.includes('function isViewerSculptureHitBetweenPositions('), 'Sculpture sweep test missing');
assert(source.includes('galleryExhibitSegmentIntersectsExpandedAabb2D('), 'Sculpture proxy is not swept in XZ');
const obstacleResolver = extractFunction(source, 'isViewerObstacleHitBetweenPositions');
assert(obstacleResolver.includes('isViewerWallHitBetweenPositions') && obstacleResolver.includes('isViewerSculptureHitBetweenPositions'), 'Custom resolver does not combine walls and sculptures');
const safePosition = extractFunction(source, 'isViewerCameraPositionSafeAgainstWalls');
assert(safePosition.includes('isViewerSculptureTooCloseAtPosition(position, viewerWallLastSafeCameraPosition)'), 'Sculpture endpoint overlap check missing');
const wallResolver = extractFunction(source, 'resolveViewerWallCollisionAfterMovement');
assert(wallResolver.includes('isViewerCustomBlockActive()'), 'Custom post-move resolver does not activate for sculpture-only collision');
assert(wallResolver.includes('isViewerCameraPositionSafeAgainstWalls(slideX)') && wallResolver.includes('isViewerCameraPositionSafeAgainstWalls(slideZ)'), 'Sliding bypasses sculpture proxies');

// Persistent Local Lights: no camera/frustum/zone culling or runtime intensity fading remains.
assert(source.includes('function applyLocalLightUserState(item)'), 'Persistent Local Light state applicator missing');
for (const removed of ['localLightCameraCullingEnabled', 'updateLocalLightsCameraCulling', 'runtimeTargetIntensity', 'markLocalLightCameraCullingDirty']) {
  assert(!source.includes(removed), `Obsolete Local Light culling path remains: ${removed}`);
}

// Existing Edit workflow, save integrity and accepted popup remain protected.
assert(index.includes('#siteHeader #saveStateButton { display: none !important; }'), 'Header Save can flash before Edit Mode');
for (const tab of ['EXHIBITS', 'SPACE', 'LIGHTING', 'SETTINGS']) assert(source.includes(`label: "${tab}"`), `Missing ${tab} tab`);
...
assert(minified.includes('12C66C3'), 'Production Stage 12C66C3 identity missing');
...
console.log('Stage 12C66C3 verifier passed.');

...
expect('Scene factory accepts external runtime options',
  ...
  bootstrap.includes('createSceneLoadingOrchestrator({') &&
...
expect('Admin Workspace stage identity exists',
  ...
  adminBootstrap.includes('const STAGE = "V14.1.2"'));
...
expect('Engine has explicit Admin Workspace runtime mode through canonical V14.1.2 policy/orchestrator compatibility flags',
  ...
  source.includes('var galleryAdminWorkspaceMode = galleryLegacySceneModeFlags.adminWorkspace === true && !galleryAuthoringSpacePreview;') &&
...
expect('Runtime identity preserves C6C8C4 residency under current C6C8C25 build', source.includes('Stage 12C66C6C8C4: Space Residency / Exhibition Delta Switch') && source.includes('C6C8C25: Cross-Space Runtime') && pkg.version.includes('v14-1-2-scene-loading-orchestrator'));
...
expect('Current runtime/package identity preserves C6C8C5 under C6C8C25', source.includes('stage: "C6C8C21"') && pkg.version.includes('v14-1-2-scene-loading-orchestrator'));

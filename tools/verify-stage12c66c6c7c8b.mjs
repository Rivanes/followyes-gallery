...
const txt=fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66C6C7C8B_ADMIN_WORKSPACE_LOGIN_DISABLED.txt',import.meta.url),'utf8');
...
assert(index.includes('stage: "12C66C6C7C8B"'),'Index stage identity missing');
assert(bootstrap.includes('const STAGE = "12C66C6C7C8B"'),'Bootstrap stage identity missing');
assert(bootstrap.includes('stage12c66c6c7c8b_admin_workspace_20260812'),'C6C7/C6C8 cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66c6c7c8b_admin_workspace_20260812'),'Index cache key missing');
assert(editorBootstrap.includes('Stage 12C66C6C7C8B'),'Editor bootstrap label missing');
...
assert(minified.includes('galleryArtworkFrameStorageFolder')&&minified.includes('gallery-artwork-residency.v1')&&minified.includes('gallery-mobile-survival-snapshot.v1'),'C6C7/C6C8B runtime missing from production build');
...
assert(bootstrap.includes('exhibitionId: getRequestedExhibitionId()'), 'Bootstrap exhibition selection missing');
...
console.log('Stage 12C66C6C7C8B verifier passed.');

...
const txt = fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66C6B_AVIF_PIPELINE_MIGRATION_WEBP_REMOVAL_LOGIN_DISABLED.txt', import.meta.url), 'utf8');
...
assert(index.includes('stage: "12C66C6B"'), 'Boot Guard C5 identity missing');
assert(bootstrap.includes('const STAGE = "12C66C6B"'), 'Viewer runtime C5 identity missing');
assert(bootstrap.includes('stage12c66c6b_avif_pipeline_migration_webp_removal_20260724'), 'C6B cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66c6b_avif_pipeline_migration_webp_removal_20260724'), 'Index C6B cache key missing');
assert(editorBootstrap.includes('Stage 12C66C6B'), 'Editor bootstrap C6B label missing');
...
assert(source.includes('stage: "12C66C6B"'), 'C6B runtime identity missing');
...
assert(minified.includes('12C66C6B') && minified.includes('gallery-ground-collision.v1') && minified.includes('gallery-sculpture-core.v2'), 'C5 runtime missing from production build');
...
assert(txt.includes('var galleryEditorLoginEnabled = false;'), 'Login-disabled C6B build missing');
assert(!txt.includes('var galleryEditorLoginEnabled = true;'), 'Login remains enabled in C6B TXT');

console.log('Stage 12C66C6B verifier passed.');

...
const txt = fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66C6A1_INSPECT_TRANSITION_COMPACT_MOBILE_LOGIN_DISABLED.txt', import.meta.url), 'utf8');
...
assert(index.includes('stage: "12C66C6A1"'), 'Boot Guard C5 identity missing');
assert(bootstrap.includes('const STAGE = "12C66C6A1"'), 'Viewer runtime C5 identity missing');
assert(bootstrap.includes('stage12c66c6a1_inspect_transition_compact_mobile_20260724'), 'C6A1 cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66c6a1_inspect_transition_compact_mobile_20260724'), 'Index C6A1 cache key missing');
assert(editorBootstrap.includes('Stage 12C66C6A1'), 'Editor bootstrap C5 label missing');
...
assert(source.includes('stage: "12C66C6A1"'), 'C6A1 Inspect runtime identity missing');
...
assert(minified.includes('12C66C6A1') && minified.includes('gallery-ground-collision.v1') && minified.includes('gallery-sculpture-core.v2'), 'C5 runtime missing from production build');
...
console.log('Stage 12C66C6A1 verifier passed.');

...
function createHarness({
  ...
  const runtime = {
    stage: '12C66B2R', schema: 'gallery-save-integrity.v2', sessionId: 'test-session',
  ...
  return { context, calls, messages, storage, draftState, runtime };
}
...
// Previous revision asset remains queued and is not deleted after the save that creates main_previous.
{
  ...
  const { context, calls, runtime } = createHarness({ serverState });
...
{
  const { context, calls, runtime } = createHarness();
  runtime.pendingStorageDeletes = [
...
{
  const { context, calls, runtime, draftState } = createHarness();
  runtime.pendingStorageDeletes = [
...
// Failed main save never starts Storage cleanup.
{
  ...
  const { context, calls, runtime } = createHarness({ serverState, mainWriteError: { message: 'network' } });
  runtime.pendingStorageDeletes = [
    { bucket: 'gallery-artworks', path: 'main/old.jpg', kind: 'artwork-image' }
  ];
  const ok = await context.saveGalleryStateToSupabase();
...
console.log('Stage 12C66B2R save-integrity repair tests passed.');

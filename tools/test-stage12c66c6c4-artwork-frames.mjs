...
console.log('Artwork frame C6C4 invariants passed.');

expect('Frame fitting uses detected inner opening instead of outer bounds',
  source.includes('detectArtworkFrameInnerOpening(meshes, centeredBounds)') &&
  source.includes('runtime.referenceOpeningWidth || runtime.referenceWidth') &&
  source.includes('runtime.referenceOpeningHeight || runtime.referenceHeight'));

expect('Frame back face is normalized to Z zero and seated on artwork front',
  source.includes('orientationRoot.position.z -= orientedBounds.min.z;') &&
  source.includes('var artworkFrontOffset = artworkDepth * artworkDepthScale * 0.5;') &&
  !source.includes('((runtime.referenceDepth || 0.04) * 0.5) + galleryArtworkFrameSurfaceEpsilon'));

expect('Frame receives required local Z 180 degree rotation',
  source.includes('orientationRoot.rotation.z += Math.PI;'));

expect('Frame catalog warms shared GLBs in parallel',
  source.includes('function warmGalleryArtworkFrameCatalog()') &&
  source.includes('Promise.all(urls.map(function (url)') &&
  source.includes('warmGalleryArtworkFrameCatalog();'));

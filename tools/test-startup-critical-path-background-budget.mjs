...
expect('package stage', pkg.version.includes('c6c8c10'));
expect('runtime stage', source.includes('stage: "12C66C6C8C10"') && source.includes('exhibition-platform-multi-exhibition.v10'));
expect('bounded foreground artwork budget', source.includes('foregroundArtworkLimit: galleryDeviceProfile && galleryDeviceProfile.mobile ? 4 : 6') && source.includes('function prepareGalleryForegroundArtworkBudget('));
...
expect('cached batched Space warmup', source.includes('gallerySpaceGpuWarmMaterialCache') && source.includes('batchSize = galleryDeviceProfile.mobile ? 1 : 3') && source.includes('Promise.all(pairs.slice(i, i + batchSize)'));
expect('admin background diagnostics', admin.includes('BG slices') && admin.includes('foreground readiness waits only for Space plus a bounded foreground Preview set'));

...
const txt=fs.readFileSync(new URL('../Gallery_V0_11_STAGE12C66C6C3_ARTWORK_FRAME_LIBRARY_LOGIN_DISABLED.txt',import.meta.url),'utf8');
...
assert(index.includes('stage: "12C66C6C3"'),'Index stage identity missing');
assert(bootstrap.includes('const STAGE = "12C66C6C3"'),'Bootstrap stage identity missing');
assert(bootstrap.includes('stage12c66c6c3_artwork_frame_library_20260812'),'C6C3 cache key missing');
assert(index.includes('gallery-viewer-bootstrap.js?v=stage12c66c6c3_artwork_frame_library_20260812'),'Index cache key missing');
assert(editorBootstrap.includes('Stage 12C66C6C3'),'Editor bootstrap label missing');
assert(source.includes('Stage 12C66C6C2: Mobile Memory Survival / Tiered Artwork Residency'),'Base source history missing');
assert(source.includes('Stage 12C66C6C3: Artwork Frame Library'),'C6C3 source history missing');
...
assert(minified.includes('galleryArtworkFrameStorageFolder')&&minified.includes('gallery-artwork-residency.v1')&&minified.includes('gallery-mobile-survival-snapshot.v1'),'C6C3 runtime missing from production build');
...
console.log('Stage 12C66C6C3 verifier passed.');
